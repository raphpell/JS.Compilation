...
var OneRegExpLexer =(function(){
	...
	Lexer.union({
		...
		alternative :function( aRules ){
			...
			var reResult = new RegExp ( '('+aRegExp.join( ')|(' )+')', 'g' )
			var oRule, oNode, oLanguageRule
			var fParse =function( sMatched ){
				if( ! sMatched ) return;
				...
				if( oLanguageRule ){
					...
					Lexer.nIndex += sMatched.length
					}
				return ''
				}
			return { type:'alternative', re:reResult, replacement:fParse }
			},
		rule :function( sId, m, sClassName ){
