...
NFA =function( eTree ){ // Transformation de "l'AST" en automate
	var S = eTree.nodeName
	if( Automate[S]){
		for(var a=to_array( eTree.childNodes ), i=0, eChild; eChild=a[i]; i++ ) a[i]=NFA(a[i])
		a.unshift( eTree )
		return Automate[S].apply( null, a )
		}
	if( ! eTree.oValue ) return false
	if( ! eTree.oValue.value ) throwError( 'Error: symbol='+ eTree.nodeName )
	return Automate.wrapper( eTree.oValue.value )
	}
NFAValidate =function( oNFA ){
...
DFAMinimization =function( oDFA, bPreserveFinalState ){
...
DFAToJS =function( oDFA, bWhiteSpace, bUnCompressed ){
...
DFAAggregation =function( oDFA1, oDFA2 ){
	/*
	...
	*/
	...
	oDFA = DFAMinimization( oDFA, true )
	var a = oDFA.aTokensID
	if( a && a.length ){
		var oSTATES = {}
		for(var i=0, ni=a.length; i<ni; i++){
...
DFAToRE =function( oDFA ){
	if( oDFA.I !== 1 ) throwError( "DFAToRE: L'état initial doit être égale à 1." )
